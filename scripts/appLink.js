appLink = (function(fake){
  ...
  function findCustomer(phone, email){
    ...
    ret.SetError();
    ret.Message = "No Customers Found!";
    ...
    return new Promise((resolve)=>{
      ...
      if (!_customer.IsEmpty){
        _customer.LoyaltyProgramItems = [];
        for(var i = 0; i < 10; i++){
          var n = new item('', 'Coupon for up to $20.20 off of FROMM buy 12 get average value back') // name, title, price, quantity, upc
          _customer.LoyaltyProgramItems.push(n);
        }
      }
  ...
  function getItem(input){
    ...
    for(var i = 0; i < itemRepository.length; i++){
      if (itemRepository[i].Identifier.indexOf(input) > -1){
        var item = test_addToTransaction(itemRepository[i]);
        ret.SetSuccess();
      }
    }
  ...
  async function addCustomerToTransaction_Phone(input){
    ...
    return await findCustomer(input, null);
  }
  ...
  async function addCustomerToTransaction_Email(input){
    ...
    return await findCustomer(null, input);
  }
  ...
  async function ScanHandle(value, appState){
  ...
  async function adminLaunchApplication() {
    return new Promise((res)=>{
      alert("External call to FieldStack");
      res(true);
    })
  }
  ...
  async function isTransactionInProgress() {
    return new Promise((res)=>{
      res(_customer != null); // I think if we've set the customer, we've got SOMETHING happenin!
    })
  }
